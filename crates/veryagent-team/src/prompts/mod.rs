...
pub use veryagent_team_prompts::AvailableAssistant;

use crate::types::{MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask};

fn to_prompt_role(role: crate::types::TeammateRole) -> veryagent_team_prompts::TeamPromptRole {
    match role {
        crate::types::TeammateRole::Lead => veryagent_team_prompts::TeamPromptRole::Lead,
        crate::types::TeammateRole::Teammate => veryagent_team_prompts::TeamPromptRole::Teammate,
    }
}

fn to_prompt_agent(agent: &TeamAgent) -> veryagent_team_prompts::TeamPromptAgent {
    veryagent_team_prompts::TeamPromptAgent {
...
///
/// Delegates to `veryagent-team-prompts`, the canonical Team role prompt crate.
...
pub fn build_lead_prompt(
    ...
    let body = veryagent_team_prompts::build_lead_prompt(&veryagent_team_prompts::LeadPromptParams {
        team_name,
...
pub fn build_teammate_prompt(agent: &TeamAgent, team_name: &str, members: &[TeamAgent]) -> String {
    ...
    let leader = prompt_members
        .iter()
        .find(|candidate| candidate.role == veryagent_team_prompts::TeamPromptRole::Lead)
        .cloned()
        .unwrap_or_else(|| veryagent_team_prompts::TeamPromptAgent {
            slot_id: "lead".to_owned(),
            name: "Lead".to_owned(),
            role: veryagent_team_prompts::TeamPromptRole::Lead,
            backend: agent.backend.clone(),
            model: agent.model.clone(),
            status: None,
        });
    ...
    veryagent_team_prompts::build_teammate_prompt(&veryagent_team_prompts::TeammatePromptParams {
        agent: &prompt_agent,
        team_name,
        leader: &leader,
        teammates: &teammates,
...
mod tests {
    ...
    fn lead_prompt_contains_member_list() {
        ...
        // VeryAgent bullet format: `- {name} ({backend}, status: {status})`
