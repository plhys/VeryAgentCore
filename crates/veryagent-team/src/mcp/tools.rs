use veryagent_db::models::AgentMetadataRow;
...
pub use veryagent_team_prompts::tools::{
...
pub fn all_tool_descriptors_for_role(caller_role: TeammateRole) -> Vec<ToolDescriptor> {
    veryagent_team_prompts::visible_team_tool_descriptors(caller_role == TeammateRole::Lead)
        .into_iter()
...
pub fn authorize_tool(caller_role: TeammateRole, tool_name: &str) -> Result<(), String> {
    veryagent_team_prompts::authorize_team_tool(caller_role == TeammateRole::Lead, tool_name)
}
...
pub fn is_whitelisted_backend(backend: &str) -> bool {
    veryagent_common::constants::TEAM_CAPABLE_BACKENDS.contains(&backend)
}
...
pub fn build_list_models_from_rows(
    ...
    use veryagent_common::constants::is_team_capable;
...
mod tests {
    ...
    #[test]
    fn team_spawn_agent_description_is_veryagent_original() {
        ...
        assert!(
            ...
            "description must be the full VeryAgent original, not the legacy one-liner"
        );
        assert!(
