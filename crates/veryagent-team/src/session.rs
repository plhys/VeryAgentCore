...
use veryagent_ai_agent::IWorkerTaskManager;
...
use veryagent_common::{AgentKillReason, generate_id};
...
pub struct WakeInput {
    ...
    /// as left bubbles (matches VeryAgent `TeammateManager.wake()`). These are
...
pub struct TeamSession {
    ...
    /// agent's conversation as left bubbles (VeryAgent parity: see
...
impl TeamSession {
    ...
    pub async fn start(
        ...
        let auth_token = veryagent_common::generate_id();
...
mod tests {
    ...
    use veryagent_common::{AgentKillReason, TimestampMs, now_ms};
    ...
    impl TeamProjectionMessageStore for NoopProjectionStore {
        ...
        async fn find_projected_message(
            ...
            _msg_id: &str,
            _msg_type: &str,
        ) -> Result<Option<veryagent_db::models::MessageRow>, TeamError> {
            Ok(None)
        }

        async fn insert_projected_message(&self, _row: &veryagent_db::models::MessageRow) -> Result<(), TeamError> {
            Ok(())
        }
    }
    ...
    struct RecordingProjectionStore {
        inserted: std::sync::Mutex<Vec<veryagent_db::models::MessageRow>>,
    }
    ...
    impl TeamProjectionMessageStore for RecordingProjectionStore {
        ...
        async fn insert_projected_message(&self, row: &veryagent_db::models::MessageRow) -> Result<(), TeamError> {
