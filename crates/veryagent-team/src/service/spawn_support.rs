use super::*;
use veryagent_api_types::BehaviorPolicy;
use veryagent_common::AgentType;
use veryagent_common::constants::{TEAM_CAPABLE_BACKENDS, has_mcp_capability};
use veryagent_db::models::AssistantOverlayRow;
use veryagent_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
...
/// Reuses `AgentType::full_auto_mode_id` from veryagent-common.
...
mod tests {
    ...
    use veryagent_db::models::{AssistantDefinitionRow, AssistantOverlayRow, Provider};
    use veryagent_db::{
    ...
    impl IProviderRepository for SingleProviderRepo {
        ...
        async fn create(&self, _params: veryagent_db::CreateProviderParams<'_>) -> Result<Provider, DbError> {
            Err(DbError::NotFound("not implemented".into()))
        }

        async fn update(&self, _id: &str, _params: veryagent_db::UpdateProviderParams<'_>) -> Result<Provider, DbError> {
            Err(DbError::NotFound("not implemented".into()))
        }
