use veryagent_common::now_ms;
use veryagent_db::models::{MailboxMessageRow, TeamRow, TeamTaskRow};
use veryagent_db::{DbError, ITeamRepository, UpdateTaskParams, UpdateTeamParams};
use std::sync::Mutex;
...
pub(crate) mod workspace_harness {
    ...
    use veryagent_api_types::{CreateTeamRequest, WebSocketMessage};
    use veryagent_common::{AgentKillReason, AgentType, PaginatedResult, now_ms};
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(&self, _row: &veryagent_db::models::MailboxMessageRow) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(&self, _task_id: &str, _params: &veryagent_db::UpdateTaskParams) -> Result<(), DbError> {
            Ok(())
        }
        ...
        fn new(repo: Arc<MockConversationRepo>) -> Self {
            Self {
                ...
                workspace_root: std::env::temp_dir().join(format!(
                    "veryagent-team-workspace-harness-{}",
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<veryagent_ai_agent::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: veryagent_ai_agent::types::BuildTaskOptions,
        ...
        fn collect_idle(&self, _idle_threshold_ms: veryagent_common::TimestampMs) -> Vec<String> {
            vec![]
        }
    }
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![veryagent_api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
