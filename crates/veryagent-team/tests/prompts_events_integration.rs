...
use veryagent_team::events::TeamEventEmitter;
use veryagent_team::message_projection::{
    ProjectedTeamMessage, TeamMessageProjection, TeamProjectionMessageStore, TeamProjectionRequest,
    TeamProjectionSource,
};
use veryagent_team::prompts::{AvailableAssistant, build_lead_prompt, build_teammate_prompt, build_wake_payload};
use veryagent_team::types::{
    MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask, TeammateRole, TeammateStatus,
};
use veryagent_team::visibility::TeamVisibilityPolicy;
use veryagent_team::{Mailbox, TaskBoard, TeammateManager};
...
fn lp2_lead_prompt_contains_tool_descriptions() {
    ...
    // VeryAgent lead prompt references the `team_*` coordination tools that the
