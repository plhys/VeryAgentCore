...
use veryagent_ai_agent::session_context::{
    AcpSessionBuildContext, AgentSessionContext, AgentSessionKind, ConversationContext, WorkspaceContext,
};
use veryagent_ai_agent::task_manager::AgentFactory;
use veryagent_ai_agent::types::BuildTaskOptions;
use veryagent_ai_agent::{AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
use veryagent_api_types::{AcpBuildExtra, AddAgentRequest, CreateTeamRequest, TeamAgentInput, WebSocketMessage};
use veryagent_common::{AgentKillReason, AgentType, PaginatedResult, ProviderWithModel};
use veryagent_db::models::{
...
impl FakeConversationPorts {
    fn new(repo: Arc<MockConversationRepo>) -> Self {
        let workspace_root =
            std::env::temp_dir().join(format!("veryagent-team-fake-workspaces-{}", veryagent_common::generate_id()));
        Self {
            repo,
            workspace_root,
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    ...
    ) -> Result<TeamConversationCreateResult, veryagent_team::TeamError> {
        if request.assistant_id.is_some() && request.agent_type.is_some() {
            return Err(veryagent_team::TeamError::InvalidRequest(
                "assistant-backed team conversations must not provide agent_type".into(),
            ));
        }
        let id = veryagent_common::generate_id();
        let now = veryagent_common::now_ms();
    ...
    async fn conversation_workspace(&self, conversation_id: &str) -> Result<Option<String>, veryagent_team::TeamError> {
    ...
    async fn conversation_assistant_id(&self, conversation_id: &str) -> Result<Option<String>, veryagent_team::TeamError> {
        Ok(self.repo.get_extra(conversation_id).and_then(|extra| {
            extra
                .get("assistant_id")
                .or_else(|| extra.get("preset_assistant_id"))
    ...
    async fn create_team_temp_workspace(&self, team_id: &str) -> Result<String, veryagent_team::TeamError> {
        if self
    ...
    ) -> Result<(), veryagent_team::TeamError> {
        ...
        self.repo
            .update(
                ...
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(veryagent_common::now_ms()),
                },
            )
            .await?;
        Ok(())
    ...
    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), veryagent_team::TeamError> {
    ...
    async fn warmup_agent_process(
        ...
        let row = self
            ...
            .ok_or_else(|| {
                veryagent_team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
            })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = veryagent_api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                veryagent_team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, veryagent_team::TeamError> {
        Ok(self
            .repo
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), veryagent_team::TeamError> {
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, veryagent_team::TeamError> {
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<veryagent_db::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &veryagent_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams(&self) -> Result<Vec<veryagent_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<veryagent_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, id: &str) -> Result<Option<veryagent_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, id: &str, params: &veryagent_db::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = veryagent_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, row: &veryagent_db::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<veryagent_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn create_task(&self, row: &veryagent_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<veryagent_db::models::TeamTaskRow>, DbError> {
        self.inner.find_task_by_id(team_id, task_id).await
    }
    async fn update_task(&self, task_id: &str, params: &veryagent_db::UpdateTaskParams) -> Result<(), DbError> {
        self.inner.update_task(task_id, params).await
    }
    async fn list_tasks(&self, team_id: &str) -> Result<Vec<veryagent_db::models::TeamTaskRow>, DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<veryagent_ai_agent::AgentInstance> {
    ...
    ) -> Result<veryagent_ai_agent::AgentInstance, AgentError> {
    ...
    fn collect_idle(&self, idle_threshold_ms: veryagent_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    use veryagent_ai_agent::AgentError;
    use veryagent_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use veryagent_ai_agent::protocol::events::AgentStreamEvent;
    use veryagent_ai_agent::types::SendMessageData;
    use veryagent_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), veryagent_ai_agent::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(veryagent_ai_agent::AgentInstance::Mock(Arc::new(
...
impl IProviderRepository for EmptyProviderRepo {
    async fn list(&self) -> Result<Vec<veryagent_db::models::Provider>, DbError> {
        Ok(vec![])
    }
    async fn find_by_id(&self, _id: &str) -> Result<Option<veryagent_db::models::Provider>, DbError> {
        Ok(None)
    }
    async fn create(
        &self,
        _params: veryagent_db::CreateProviderParams<'_>,
    ) -> Result<veryagent_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
    async fn update(
        &self,
        _id: &str,
        _params: veryagent_db::UpdateProviderParams<'_>,
    ) -> Result<veryagent_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
async fn ensure_session_recovery_drain_runs_agent_turn_with_team_run_id() {
    ...
    team_repo
        .write_message(&veryagent_db::models::MailboxMessageRow {
...
async fn create_team_with_workspace_writes_same_workspace_to_team_and_initial_agents() {
    ...
    let workspace_dir =
        std::env::temp_dir().join(format!("veryagent-team-user-workspace-{}", veryagent_common::generate_id()));
...
async fn tg3_get_team_rejects_cross_user_access() {
    ...
    assert!(matches!(result, Err(veryagent_team::TeamError::Forbidden(_))));
}
...
async fn aa_add_agent_inherits_team_workspace() {
    ...
    let (svc, _, conv_repo) =
        setup_with_factory_and_metadata_and_conversation_repo(success_factory(), agent_metadata_repo);
    let workspace = std::env::temp_dir().join(format!("veryagent-team-workspace-{}", veryagent_common::generate_id()));
...
async fn add_agent_uses_team_temp_workspace_when_team_and_leader_workspaces_are_unusable() {
    ...
    conv_repo
        .patch_extra(
            ...
            serde_json::json!({ "workspace": "/tmp/veryagent-team-missing-leader-workspace" }),
        )
        .unwrap();
...
async fn d31b2_ensure_session_broadcasts_load_failed_for_missing_team() {
    ...
    assert!(matches!(err, veryagent_team::TeamError::TeamNotFound(_)));
