...
use veryagent_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use veryagent_common::{TimestampMs, now_ms};
use veryagent_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
const DEFAULT_CLIENT_ID: &str = "veryagent";
...
impl McpOAuthService {
    ...
    pub async fn logout(&self, server_url: &str) -> Result<(), McpError> {
        match self.token_repo.delete(server_url).await {
            ...
            Err(veryagent_db::DbError::NotFound(_)) => {
    ...
    async fn handle_callback_connection(
        ...
        let response = "HTTP/1.1 200 OK\r\n\
            ...
            <p>You can close this window and return to VeryAgent.</p>\
            </body></html>";
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<veryagent_db::models::OAuthTokenRow>, veryagent_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<veryagent_db::models::OAuthTokenRow, veryagent_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str) -> Result<(), veryagent_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self) -> Result<Vec<String>, veryagent_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), veryagent_db::DbError> {
            Err(veryagent_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<veryagent_db::models::OAuthTokenRow>, veryagent_db::DbError> {
            Ok(Some(veryagent_db::models::OAuthTokenRow {
