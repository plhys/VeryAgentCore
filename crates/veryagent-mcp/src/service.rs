...
use veryagent_db::{CreateMcpServerParams, IMcpServerRepository, UpdateMcpServerParams};
...
mod tests {
    use super::*;
    use veryagent_api_types::{ImportMcpServerRequest, McpTransport};
    use veryagent_common::{McpServerStatus, TimestampMs};
    use veryagent_db::models::McpServerRow;
    use veryagent_db::{CreateMcpServerParams, DbError, UpdateMcpServerParams};
    ...
    async fn persist_test_result_records_success_status_and_tools() {
        ...
        let result = McpConnectionTestResult {
            success: true,
            tools: Some(vec![veryagent_api_types::McpToolResponse {
                name: "read_file".into(),
        ...
        assert_eq!(updated.last_test_status, veryagent_common::McpServerStatus::Connected);
    ...
    async fn persist_test_result_records_error_and_clears_tools() {
        ...
        let updated = svc.get_server(&created.id).await.unwrap();
        assert_eq!(updated.last_test_status, veryagent_common::McpServerStatus::Error);
