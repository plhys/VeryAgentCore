...
use veryagent_api_types::{BedrockConfig, FetchModelsAnonymousRequest, FetchModelsRequest, FetchModelsResponse};
use veryagent_common::decrypt_string;
use veryagent_db::IProviderRepository;
...
mod tests {
    use super::*;
    use veryagent_common::encrypt_string;
    use veryagent_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];

    async fn setup() -> (ModelFetchService, veryagent_db::Database) {
    ...
    async fn create_provider(db: &veryagent_db::Database, platform: &str, base_url: &str, api_key: &str) -> String {
