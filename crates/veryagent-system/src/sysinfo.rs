use veryagent_api_types::SystemInfoResponse;
...
///
/// Priority: `VERYAGENT_CACHE_DIR` env → `dirs::cache_dir()/veryagent`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("VERYAGENT_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("veryagent").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for AionUI.
///
/// Priority: `VERYAGENT_WORK_DIR` env → `dirs::data_dir()/veryagent`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("VERYAGENT_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
///
/// Priority: `VERYAGENT_LOG_DIR` env →
///   macOS: `~/Library/Logs/veryagent`
///   Linux: `dirs::state_dir()/veryagent/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/veryagent/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("VERYAGENT_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/veryagent").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("veryagent/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/veryagent/logs
    dirs::data_dir()
        .map(|p| p.join("veryagent/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "veryagent".
        let dir = resolve_cache_dir();
        assert!(dir.contains("veryagent"), "cache_dir should contain 'veryagent': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("veryagent"), "work_dir should contain 'veryagent': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("veryagent"), "log_dir should contain 'veryagent': {dir}");
    }
}
