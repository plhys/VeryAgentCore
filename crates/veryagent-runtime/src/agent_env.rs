...
mod tests {
    ...
    const CHILD_MARKER: &str = "VERYAGENT_RUNTIME_AGENT_ENV_TEST_CHILD";
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("VERYAGENT_CURRENT_ONLY", "from-current")
                .env("VERYAGENT_OVERLAY", "from-current")
        ...
        assert_eq!(value("VERYAGENT_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("VERYAGENT_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("VERYAGENT_OVERLAY").as_deref(), Some("from-shell"));
