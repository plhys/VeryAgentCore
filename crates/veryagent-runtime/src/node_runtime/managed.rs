...
mod tests {
    ...
    fn classify_error_detects_bundled_node_runtime_missing() {
        let err = NodeRuntimeError::managed_invalid(
            "bundled Node runtime missing under C:\\Program Files\\VeryAgent\\resources\\bundled-aioncore\\win32-x64\\managed-resources\\node\\node-v24.11.0-win-x64",
        );
    ...
    async fn bundled_runtime_missing_reports_bundled_resource_missing() {
        ...
        if !crate::test_support::run_in_env_child(
            ...
            |command| {
                command.env("VERYAGENT_BUNDLED_MANAGED_RESOURCES", &bundled_root);
            },
        ) {
            return;
        }
    ...
    fn managed_runtime_install_lock_path_uses_runtime_root() {
        let root = PathBuf::from("/tmp/veryagent/runtime/node");
    ...
    async fn bundled_runtime_validation_failure_does_not_fallback_to_remote_download() {
        ...
        let bundled_root = std::path::PathBuf::from(std::env::var_os("VERYAGENT_BUNDLED_MANAGED_RESOURCES").unwrap());
