//! Access to bun metadata emitted at build time.
//!
//! AionCore no longer embeds bun/bunx. `build.rs` always emits a stub with
//! `HAS_EMBEDDED_BUN = false`, so production falls back to an explicit
//! `VERYAGENT_BUN_PATH` override or a system-installed `bun`.
...
mod tests {
    ...
    #[test]
    fn production_embed_is_stubbed() {
