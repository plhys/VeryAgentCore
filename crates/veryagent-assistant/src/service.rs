...
use veryagent_common::{generate_prefixed_id, now_ms};
use veryagent_db::{
...
use veryagent_extension::{AssistantClassifier, AssistantRuleDispatcher, ExtensionError};
use serde_json;
...
pub struct AssistantService {
    ...
    /// Defaults to `~/.veryagent/` but can be overridden for tests.
...
impl AssistantService {
    ...
    /// directory.
    ///
    /// There is no implicit `~/.veryagent` fallback on purpose: an earlier
    /// version had one, and dev builds silently wrote rule files to the
    /// release directory while the db lived under `~/.veryagent-dev/`,
    ...
    pub async fn rebuild_legacy_mirror_from_new_tables(&self) -> Result<(), AssistantError> {
        ...
        let state_map: HashMap<String, veryagent_db::AssistantOverlayRow> = states
            .into_iter()
    ...
    pub async fn import(&self, req: ImportAssistantsRequest) -> Result<ImportAssistantsResult, AssistantError> {
        ...
        for entry in req.assistants {
            ...
            match self.repo.create(&params).await {
                ...
                Err(veryagent_db::DbError::Conflict(_)) => {
    ...
    ///   override when `VERYAGENT_BUILTIN_ASSISTANTS_PATH` is set).
...
fn definition_to_detail_response(
    definition: &AssistantDefinitionRow,
    state: Option<&AssistantOverlayRow>,
    preference: Option<&veryagent_db::AssistantPreferenceRow>,
    rules_content: &str,
    projection: &AssistantRuntimeProjection,
...
mod tests {
    ...
    struct Fixture {
        ...
        preference_repo: Arc<dyn IAssistantPreferenceRepository>,
        provider_repo: Arc<dyn IProviderRepository>,
        agent_rows: Arc<Mutex<Vec<veryagent_api_types::AgentManagementRow>>>,
        _tmp: TempDir,
        _db: veryagent_db::Database,
    }

    #[derive(Clone, Default)]
    struct StubAgentCatalog {
        rows: Arc<Mutex<Vec<veryagent_api_types::AgentManagementRow>>>,
    }
    ...
    struct FixtureOpts {
        ...
        agent_rows: Vec<veryagent_api_types::AgentManagementRow>,
    }
    ...
    fn mk_agent_row(
        id: &str,
        backend: &str,
        status: veryagent_api_types::AgentManagementStatus,
    ) -> veryagent_api_types::AgentManagementRow {
        veryagent_api_types::AgentManagementRow {
            id: id.into(),
            ...
            behavior_policy: veryagent_api_types::BehaviorPolicy {
                supports_team: true,
                ..Default::default()
            },
            yolo_id: None,
            ...
            team_capable: true,
            status,
            last_check_status: Some(veryagent_api_types::AgentSnapshotCheckStatus::Online),
            last_check_kind: Some(veryagent_api_types::AgentSnapshotCheckKind::Manual),
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_agent() {
        let fx = fixture_with_options(FixtureOpts {
            agent_rows: vec![mk_agent_row(
                "agent-claude",
                "claude",
                veryagent_api_types::AgentManagementStatus::Online,
            )],
            ..Default::default()
        })
        .await;
        ...
        assert_eq!(bare.agent_status, veryagent_api_types::AgentManagementStatus::Online);
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_custom_agent() {
        ...
        custom_row.agent_source = veryagent_api_types::AgentSource::Custom;
    ...
    async fn bootstrap_falls_back_to_agent_type_when_backend_is_empty() {
        ...
        agent_row.agent_type = veryagent_common::AgentType::Aionrs;
    ...
    async fn aionrs_assistant_resolves_agent_status_via_agent_type_not_backend() {
        ...
        aionrs_row.agent_type = veryagent_common::AgentType::Aionrs;
    ...
    async fn bootstrap_places_new_generated_assistants_before_existing_assistants() {
        let fx = fixture_with_options(FixtureOpts {
            ...
            agent_rows: vec![
                ...
                mk_agent_row("agent-codex", "codex", veryagent_api_types::AgentManagementStatus::Online),
            ],
            ..Default::default()
        })
        .await;
