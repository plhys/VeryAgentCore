...
use veryagent_extension::hub::{HubIndexManager, HubInstaller};
use veryagent_extension::registry::ExtensionRegistry;
use veryagent_extension::state::ExtensionStateStore;
use veryagent_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![veryagent_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: veryagent_extension::types::ExtensionSource::Env,
    }];
