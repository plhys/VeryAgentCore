...
//!
//! These tests exercise the public API surface of `veryagent_extension::skill_service`
//! and `veryagent_extension::external_paths` against the functional requirements in
//! `08-file-workspace.md` §B.

use std::path::Path;

use veryagent_extension::external_paths::ExternalPathsManager;
use veryagent_extension::skill_service::{
...
///
/// Production mode: no `VERYAGENT_BUILTIN_SKILLS_PATH` set — the built-in
...
async fn sm11_get_skill_paths() {
    ...
    let data_dir = Path::new("/home/user/.veryagent");
...
async fn cp4_enable_skills_market() {
    ...
    let paths = mgr.get_custom_external_paths().await;
    assert_eq!(paths.len(), 1);
    assert_eq!(paths[0].name, "veryagent-skills");
}
