...
use veryagent_api_types::{ApiResponse, AssistantSource, SkillImportLimitsResponse};
use veryagent_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use veryagent_extension::error::ExtensionError;
use veryagent_extension::external_paths::ExternalPathsManager;
use veryagent_extension::skill_routes::{SkillRouterState, skill_routes};
use veryagent_extension::skill_service::SkillPaths;
use axum::body::Body;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let skill_repo = Arc::new(veryagent_db::SqliteSkillRepository::new(db.pool().clone()));
    std::mem::forget(tmp);
