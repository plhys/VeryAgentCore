...
use veryagent_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
///
/// Mirrors the strategy used by `veryagent-assistant::builtin`: the corpus is
/// authoritative at build time; an optional on-disk override
/// (`VERYAGENT_BUILTIN_SKILLS_PATH`) is consulted at runtime for rapid
/// iteration and E2E fixtures.
static BUILTIN_SKILLS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/../veryagent-app/assets/builtin-skills");
...
pub const BUILTIN_SKILLS_ENV_VAR: &str = "VERYAGENT_BUILTIN_SKILLS_PATH";
...
pub struct SkillPaths {
    /// Root data directory (~/.veryagent/).
    pub data_dir: PathBuf,
    /// User-created skills directory (~/.veryagent/skills/).
    pub user_skills_dir: PathBuf,
    /// Per-job cron skills directory (~/.veryagent/cron/skills/).
    ...
    /// Assistant-level rules directory (~/.veryagent/assistant-rules/).
    pub assistant_rules_dir: PathBuf,
    /// Assistant-level skills directory (~/.veryagent/assistant-skills/).
...
///
/// `data_dir` is the user-level data root (e.g. `~/.veryagent/`) and
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = veryagent_common::generate_prefixed_id("skill_import_op");
...
async fn import_skills_from_zip(paths: &SkillPaths, archive_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    ...
    let result = async {
        ...
        import_skill_dirs_batch(
            paths,
            skill_dirs,
            veryagent_common::generate_prefixed_id("skill_import_op"),
        )
        .await
    }
    .await;
...
mod tests {
    use super::*;
    use veryagent_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
    use std::io::Write;
