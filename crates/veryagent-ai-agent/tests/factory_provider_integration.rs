...
use veryagent_ai_agent::AcpSessionSyncService;
use veryagent_ai_agent::AcpSkillManager;
use veryagent_ai_agent::factory::{AgentFactoryDeps, build_agent_factory};
...
use veryagent_api_types::AionrsBuildExtra;
use veryagent_common::{AgentType, ProviderWithModel, encrypt_string};
use veryagent_db::{
...
) -> veryagent_ai_agent::task_manager::AgentFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(veryagent_extension::resolve_skill_paths(tmp.path(), tmp.path()));
