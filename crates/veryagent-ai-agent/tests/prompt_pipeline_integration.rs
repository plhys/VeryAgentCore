...
use veryagent_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use veryagent_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use veryagent_ai_agent::manager::acp::{AcpSession, SessionNewPreludeHook};
use veryagent_ai_agent::registry::AgentRegistry;
use veryagent_ai_agent::shared_kernel::ModelId;
use veryagent_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(veryagent_extension::resolve_skill_paths(tmp.path(), tmp.path()));
