...
use veryagent_api_types::{AgentManagementStatus, AgentSnapshotCheckKind, AgentSnapshotCheckStatus};
use veryagent_db::{
...
async fn management_rows_derive_missing_available_and_unavailable_statuses() {
    ...
    repo.upsert(&custom_params(
        "agent-missing",
        "Missing Agent",
        "veryagent-missing-agent-binary",
        r#"{"binary_name":"veryagent-missing-agent-binary"}"#,
    ))
    .await
    .unwrap();
    repo.upsert(&custom_params(
