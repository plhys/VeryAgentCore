...
use veryagent_db::{AgentMetadataRow, IAgentMetadataRepository, UpdateAgentHandshakeParams};
...
fn decode_row(row: AgentMetadataRow) -> Option<(AgentMetadata, Option<UnavailableReason>)> {
    ...
    let team_capable = behavior_policy.supports_team
        || veryagent_common::constants::is_team_capable(backend_str, handshake.agent_capabilities.as_ref());
...
/// agent's runtime environment or VeryAgent-internal wiring. Case-insensitive.
pub(crate) fn is_blocked_override_env_key(key: &str) -> bool {
    let upper = key.to_ascii_uppercase();
    if upper.starts_with("VERYAGENT_") {
        return true;
    }
    matches!(
        upper.as_str(),
...
mod tests {
    ...
    async fn codex_yolo_id_maps_to_full_access() {
        ...
        // Legacy VeryAgent yolo aliases resolve to Codex's native
    ...
    fn decode_row_applies_command_override() {
        use veryagent_db::AgentMetadataRow;
