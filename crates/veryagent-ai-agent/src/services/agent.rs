...
//! `veryagent-conversation::ConversationService`, which dispatches through
...
impl AgentService {
    ...
    pub async fn set_agent_overrides(
        &self,
        id: &str,
        req: veryagent_api_types::SetAgentOverridesRequest,
    ...
    pub async fn get_agent_overrides(&self, id: &str) -> Result<veryagent_api_types::AgentOverridesResponse, AgentError> {
        let row = self
            .registry
            .repo_handle()
            .get(id)
        ...
        let env_override = row
            .env_override
            .as_deref()
            .and_then(|s| serde_json::from_str::<Vec<veryagent_api_types::AgentEnvEntry>>(s).ok())
            .unwrap_or_default();

        Ok(veryagent_api_types::AgentOverridesResponse {
