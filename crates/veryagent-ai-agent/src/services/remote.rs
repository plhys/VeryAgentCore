...
use veryagent_common::{RemoteAgentAuthType, RemoteAgentProtocol, RemoteAgentStatus, decrypt_string, encrypt_string};
use veryagent_db::models::RemoteAgentRow;
use veryagent_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
use base64::Engine;
...
impl RemoteAgentService {
    ...
    pub async fn create(&self, req: CreateRemoteAgentRequest) -> Result<RemoteAgentResponse, AgentError> {
        ...
        let row = self
            .repo
            .create(veryagent_db::CreateRemoteAgentParams {
                name: &req.name,
    ...
    pub async fn update(&self, id: &str, req: UpdateRemoteAgentRequest) -> Result<RemoteAgentResponse, AgentError> {
        ...
        let row = self.repo.update(id, params).await.map_err(|e| match e {
            veryagent_db::DbError::NotFound(msg) => AgentError::not_found(msg),
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AgentError> {
    ...
    let device_id = veryagent_common::generate_prefixed_id("dev");
...
fn db_err(e: veryagent_db::DbError) -> AgentError {
