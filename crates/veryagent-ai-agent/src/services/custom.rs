...
use veryagent_common::generate_short_id;
...
async fn probe_or_reject(req: &CustomAgentUpsertRequest, data_dir: &Path) -> Result<(), AgentError> {
    ...
    if std::env::var("VERYAGENT_BYPASS_PROBE").is_ok() {
        tracing::warn!("VERYAGENT_BYPASS_PROBE set — skipping custom agent probe. Test-only.");
        return Ok(());
    }
