...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<veryagent_api_types::AgentModeResponse, AgentError> {
        Ok(veryagent_api_types::AgentModeResponse {
            mode: "default".into(),
            initialized: false,
        })
    }
...
impl AgentInstance {
    ...
    pub fn get_confirmations(&self) -> Vec<veryagent_common::Confirmation> {
        match self {
    ...
    pub async fn get_mode(&self) -> Result<veryagent_api_types::AgentModeResponse, AgentError> {
        match self {
    ...
    /// [`veryagent_common::normalize_keys_to_snake_case`] so keys land as
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AgentError> {
        match self {
            Self::Acp(m) => {
                ...
                veryagent_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
...
mod aionrs_config_option_tests {
    ...
    use veryagent_api_types::ConfigOptionConfirmation;
