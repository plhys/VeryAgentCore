...
use veryagent_api_types::{SessionMcpServer, SessionMcpTransport};
use veryagent_common::CommandSpec;
use veryagent_db::IMcpServerRepository;
use veryagent_db::models::McpServerRow;
use veryagent_mcp::{AcpMcpCapabilities, parse_acp_mcp_capabilities};
    ...
    if meta.backend.as_deref() == Some("claude") {
        ...
        if !cc_switch_env.is_empty() {
            ...
            for (name, value) in &cc_switch_env {
                command_spec.env.push(veryagent_common::EnvVar {
                    name: name.clone(),
                    value: value.clone(),
                });
            }
...
async fn resolve_agent_command_spec(
    meta: &veryagent_api_types::AgentMetadata,
    workspace: &str,
    conversation_id: &str,
    broadcaster: Arc<dyn veryagent_realtime::EventBroadcaster>,
) -> Result<CommandSpec, AgentError> {
    if meta.agent_source == veryagent_api_types::AgentSource::Builtin
    ...
    let mut env: Vec<veryagent_common::EnvVar> = meta
        .env
        .iter()
        .map(|entry| veryagent_common::EnvVar {
            name: entry.name.clone(),
            value: entry.value.clone(),
        })
        .collect();
    env.extend(resolved.env.iter().map(|(name, value)| veryagent_common::EnvVar {
...
mod tests {
    use super::*;
    use veryagent_realtime::BroadcastEventBus;
    use veryagent_runtime::init as init_runtime;
    ...
    async fn row_to_sdk_stdio_flattens_resolved_npx_command() {
        ...
        unsafe { std::env::set_var("VERYAGENT_BUNDLED_MANAGED_RESOURCES", runtime.path()) };
        ...
        unsafe { std::env::remove_var("VERYAGENT_BUNDLED_MANAGED_RESOURCES") };
        match server {
            McpServer::Stdio(s) => {
    ...
    async fn resolve_agent_command_spec_flattens_bare_npx_command() {
        ...
        let meta = veryagent_api_types::AgentMetadata {
            id: "agent-1".into(),
            icon: None,
            name: "Test ACP".into(),
            name_i18n: None,
            ...
            agent_source: veryagent_api_types::AgentSource::Custom,
            ...
            env: vec![veryagent_api_types::AgentEnvEntry {
                name: "K".into(),
                value: "V".into(),
                description: None,
            }],
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self) -> Result<Vec<McpServerRow>, veryagent_db::DbError> {
            if self.fail {
                Err(veryagent_db::DbError::Init("simulated".into()))
            } else {
                Ok(self.rows.clone())
            }
        }
        async fn find_by_id(&self, _id: &str) -> Result<Option<McpServerRow>, veryagent_db::DbError> {
            unimplemented!()
        }
        async fn find_by_name(&self, _name: &str) -> Result<Option<McpServerRow>, veryagent_db::DbError> {
            unimplemented!()
        }
        async fn list_by_ids_any(&self, ids: &[String]) -> Result<Vec<McpServerRow>, veryagent_db::DbError> {
            if self.fail {
                return Err(veryagent_db::DbError::Init("simulated".into()));
            }
            Ok(ids
                .iter()
        ...
        async fn create(
            &self,
            _params: veryagent_db::CreateMcpServerParams<'_>,
        ) -> Result<McpServerRow, veryagent_db::DbError> {
            unimplemented!()
        }
        async fn update(
            &self,
            _id: &str,
            _params: veryagent_db::UpdateMcpServerParams<'_>,
        ) -> Result<McpServerRow, veryagent_db::DbError> {
            unimplemented!()
        }
        ...
        async fn batch_upsert(
            &self,
            _servers: &[veryagent_db::CreateMcpServerParams<'_>],
        ) -> Result<Vec<McpServerRow>, veryagent_db::DbError> {
            unimplemented!()
        }
        async fn update_status(
            ...
            _id: &str,
            _status: &str,
            _last_connected: Option<veryagent_common::TimestampMs>,
        ) -> Result<(), veryagent_db::DbError> {
            unimplemented!()
        }
        async fn update_tools(&self, _id: &str, _tools: Option<&str>) -> Result<(), veryagent_db::DbError> {
            unimplemented!()
        }
    }
