...
pub(super) async fn build(
    ...
    let api_key = veryagent_common::decrypt_string(&row.api_key_encrypted, &deps.encryption_key)
...
/// Map VeryAgent DB platform name to the aionrs provider identifier.
///
...
pub(crate) fn resolve_bedrock_config(json: Option<&str>) -> Option<aion_config::config::BedrockConfig> {
    let bc: veryagent_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
...
mod tests {
    ...
    impl IMcpServerRepository for MockMcpRepo {
        ...
        async fn find_by_id(&self, id: &str) -> Result<Option<McpServerRow>, veryagent_db::DbError> {
            Ok(self.rows.iter().find(|row| row.id == id).cloned())
        }

        async fn find_by_name(&self, name: &str) -> Result<Option<McpServerRow>, veryagent_db::DbError> {
