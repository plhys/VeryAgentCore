use veryagent_common::{CommandSpec, ErrorChain};
use veryagent_runtime::Builder as CmdBuilder;
use std::path::Path;
use std::sync::Arc;
...
impl CliAgentProcess {
    ...
    pub async fn spawn_for_sdk(config: CommandSpec, data_dir: &Path) -> Result<Self, AgentError> {
        ...
        let agent_env = veryagent_runtime::agent_process_env().await;
...
mod tests {
    ...
    fn agent_spawn_env_does_not_override_claude_code_executable_from_path() {
        const CHILD_ENV: &str = "VERYAGENT_TEST_AGENT_SPAWN_ENV_CHILD";
    ...
    async fn spawn_for_sdk_uses_clean_agent_env_and_explicit_overrides() {
        const CHILD_ENV: &str = "VERYAGENT_TEST_SDK_AGENT_ENV_CHILD";
        ...
        if std::env::var_os(CHILD_ENV).is_none() {
            ...
            write_fake_shell(
                ...
                r#"#!/bin/sh
printf '%s\n' \
  'VERYAGENT_SHELL_ONLY=from-shell' \
  'VERYAGENT_OVERLAY=from-shell' \
...
"#,
        ...
        let mut config = simple_script_config(
            "printf 'shell=%s\nconfig=%s\noverlay=%s\nnpm=%s\nnode=%s\nbun=%s\n' \
             \"${VERYAGENT_SHELL_ONLY:-unset}\" \
             \"${VERYAGENT_CONFIG_ONLY:-unset}\" \
             \"${VERYAGENT_OVERLAY:-unset}\" \
             \"${npm_lifecycle_event:-unset}\" \
             \"${NODE_OPTIONS:-unset}\" \
             \"${BUN_INSTALL_CACHE_DIR:-unset}\"",
        );
        config.env.push(EnvVar {
            name: "VERYAGENT_CONFIG_ONLY".into(),
            value: "from-config".into(),
        });
        config.env.push(EnvVar {
            name: "VERYAGENT_OVERLAY".into(),
            value: "from-config".into(),
        });
