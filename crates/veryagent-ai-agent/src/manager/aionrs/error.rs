...
mod tests {
    ...
    fn aionrs_structured_malformed_tool_call_error_is_provider_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(veryagent_api_types::AgentErrorCode::UserLlmProviderInvalidRequest)
        );
        assert_eq!(
            send_error.ownership(),
            Some(veryagent_api_types::AgentErrorOwnership::UserLlmProvider)
        );
    ...
    fn aionrs_provider_connection_error_is_user_llm_provider_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(veryagent_api_types::AgentErrorCode::UserLlmProviderNetworkError)
        );
        assert_eq!(
            send_error.ownership(),
            Some(veryagent_api_types::AgentErrorOwnership::UserLlmProvider)
    ...
    fn aionrs_provider_status_error_uses_status_instead_of_message_text() {
        ...
        assert_eq!(
            send_error.code(),
            Some(veryagent_api_types::AgentErrorCode::UserLlmProviderAuthFailed)
        );
        assert_eq!(
            send_error.ownership(),
            Some(veryagent_api_types::AgentErrorOwnership::UserLlmProvider)
    ...
    fn aionrs_context_too_long_is_provider_context_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(veryagent_api_types::AgentErrorCode::UserLlmProviderContextTooLarge)
        );
        assert_eq!(
            send_error.ownership(),
            Some(veryagent_api_types::AgentErrorOwnership::UserLlmProvider)
    ...
    fn aionrs_tool_call_failures_are_unknown_upstream_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(veryagent_api_types::AgentErrorCode::UnknownUpstreamError)
        );
        assert_eq!(
            send_error.ownership(),
            Some(veryagent_api_types::AgentErrorOwnership::UnknownUpstream)
        );
        assert_eq!(send_error.stream_error().retryable, Some(true));
    }
}
