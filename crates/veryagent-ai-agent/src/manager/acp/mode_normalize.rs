...
pub(super) fn normalize_requested_mode(metadata: &AgentMetadata, mode: &str) -> String {
    ...
    // VeryAgent persists the legacy aliases `yolo` / `yoloNoSandbox` while
...
mod tests {
    use super::*;
    use veryagent_api_types::AgentHandshake;
    use veryagent_common::AgentType;

    fn metadata_with_yolo_id(yolo_id: Option<&str>) -> AgentMetadata {
        use veryagent_api_types::{AgentSource, AgentSourceInfo, BehaviorPolicy};
        AgentMetadata {
            id: "test".into(),
            icon: None,
            name: "Test".into(),
