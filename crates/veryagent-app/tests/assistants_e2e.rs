...
//! against a real `veryagent_app::create_router_with_states` instance backed by
...
use veryagent_app::{AppConfig, AppServices, ModuleStates, build_module_states, create_router_with_states};
use veryagent_assistant::{AssistantAgentCatalogPort, AssistantRouterState, AssistantService, BuiltinAssistantRegistry};
...
impl AssistantAgentCatalogPort for TestAgentCatalog {
    async fn list_management_agents(&self) -> Result<Vec<AgentManagementRow>, veryagent_assistant::AssistantError> {
        Ok(self.rows.clone())
    }
}
...
async fn fixture() -> Fixture {
    ...
    // `build_module_states` point at `~/.veryagent/` for the state store and
    ...
    // `~/.veryagent/` for user data — neither is appropriate for tests.
    ...
    provider_repo
        .create(veryagent_db::CreateProviderParams {
            id: None,
            platform: "openai",
            name: "Test OpenAI",
