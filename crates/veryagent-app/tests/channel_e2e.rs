...
use veryagent_db::models::{AssistantSessionRow, AssistantUserRow};
use veryagent_db::{IChannelRepository, SqliteChannelRepository};
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn veryagent_db::IChannelRepository> =
        std::sync::Arc::new(veryagent_db::SqliteChannelRepository::new(pool));
    let pairing_svc = veryagent_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
