...
//! integration level in `veryagent-office/tests/`.
...
use veryagent_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use veryagent_office::{ConversionService, OfficeRouterState, OfficecliWatchManager, ProxyService, SnapshotService};
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use veryagent_office::error::OfficeError;
    use veryagent_office::types::DocType;
    use veryagent_office::{ProcessHandle, ProcessSpawner};
    ...
    impl veryagent_realtime::EventBroadcaster for NoopBroadcaster {
        fn broadcast(&self, _msg: veryagent_api_types::WebSocketMessage<serde_json::Value>) {}
    }

    let spawner: Arc<dyn ProcessSpawner> = Arc::new(NoopSpawner);
    let bc: Arc<dyn veryagent_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
