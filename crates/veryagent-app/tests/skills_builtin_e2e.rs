...
//! `veryagent_app::create_router_with_states` against an in-memory DB.

mod common;

use std::sync::Arc;

use veryagent_app::{ModuleStates, build_module_states, create_router_with_states};
use veryagent_db::init_database_memory;
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("VERYAGENT_BUILTIN_SKILLS_PATH");
    }
    ...
    veryagent_extension::materialize_if_needed(&data_dir, veryagent_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.veryagent/`,
    ...
    let skill_repo = Arc::new(veryagent_db::SqliteSkillRepository::new(services.database.pool().clone()));
    ...
    veryagent_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
...
async fn unified_skill_list_includes_auto_inject_entries_from_embedded_corpus() {
    ...
    assert!(
        !names.contains(&"veryagent-skills"),
        "veryagent-skills should not be shipped as an auto-inject builtin skill: {names:?}",
    );
