//! Shared test helpers for veryagent-app E2E tests.
#![allow(dead_code)]
...
use veryagent_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use veryagent_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use veryagent_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use veryagent_file::FileService;
use veryagent_system::VersionCheckService;
...
pub async fn build_app_with_skill_paths(root: &std::path::Path) -> (axum::Router, AppServices, SkillPaths) {
    ...
    states.skill = SkillRouterState {
        ...
        skill_repo: std::sync::Arc::new(veryagent_db::SqliteSkillRepository::new(services.database.pool().clone())),
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths) {
    let repo = veryagent_db::SqliteSkillRepository::new(services.database.pool().clone());
    veryagent_extension::sync_skill_catalog_into_repo(paths, &repo)
        .await
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await.expect("build module states");
    states.shell.shell_service = std::sync::Arc::new(veryagent_shell::ShellService::new(std::sync::Arc::new(
        veryagent_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                veryagent_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, veryagent_ai_agent::AgentError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn veryagent_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> veryagent_common::AgentType {
        veryagent_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<veryagent_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> veryagent_common::TimestampMs {
        veryagent_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<veryagent_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: veryagent_ai_agent::types::SendMessageData,
    ) -> Result<(), veryagent_ai_agent::AgentSendError> {
        Ok(())
    }
    async fn cancel(&self) -> Result<(), veryagent_ai_agent::AgentError> {
        Ok(())
    }
    fn kill(&self, _reason: Option<veryagent_common::AgentKillReason>) -> Result<(), veryagent_ai_agent::AgentError> {
        Ok(())
    }
}
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        &self,
    ) -> Result<veryagent_api_types::GetConfigOptionsResponse, veryagent_ai_agent::AgentError> {
        Ok(veryagent_api_types::GetConfigOptionsResponse {
            config_options: vec![veryagent_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                ...
                options: vec![veryagent_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<veryagent_api_types::SetConfigOptionResponse, veryagent_ai_agent::AgentError> {
        Ok(veryagent_api_types::SetConfigOptionResponse {
            confirmation: veryagent_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![veryagent_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("veryagent-csrf-token="))
        .map(|s| {
            s.strip_prefix("veryagent-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("veryagent-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = veryagent_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
