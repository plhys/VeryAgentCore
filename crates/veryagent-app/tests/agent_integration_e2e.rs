...
use veryagent_ai_agent::protocol::events::TextEventData;
use veryagent_ai_agent::types::{BuildTaskOptions, SendMessageData};
use veryagent_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
use veryagent_api_types::AgentSource;
use veryagent_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs, now_ms};
use veryagent_db::UpsertAgentMetadataParams;
...
async fn build_app_with_mock_tasks() -> (axum::Router, veryagent_app::AppServices, Arc<MockTaskManager>) {
...
async fn upsert_visible_agent_metadata(services: &veryagent_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
