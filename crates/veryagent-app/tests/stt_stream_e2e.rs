...
//! mirroring `veryagent-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
use veryagent_app::{AppConfig, AppServices, create_router};
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(veryagent_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = veryagent_system::ClientPrefService::new(repo);
    let mut req = veryagent_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the veryagent-shell integration
