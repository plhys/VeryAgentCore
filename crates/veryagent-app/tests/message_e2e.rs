...
async fn insert_message(
    services: &veryagent_app::AppServices,
    conv_id: &str,
    msg_id: &str,
    content: &str,
    created_at: i64,
    ...
    let msg = veryagent_db::models::MessageRow {
        id: msg_id.into(),
    ...
    veryagent_db::IConversationRepository::insert_message(&repo, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &veryagent_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = veryagent_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn upsert_artifact(services: &veryagent_app::AppServices, artifact: veryagent_db::ConversationArtifactRow) {
    let repo = veryagent_db::SqliteConversationRepository::new(services.database.pool().clone());
    veryagent_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap();
}
