...
use veryagent_ai_agent::{agent_routes, remote_agent_routes};
use veryagent_api_types::ErrorResponse;
use veryagent_assets::{AssetRouterState, asset_routes};
use veryagent_assistant::assistant_routes;
use veryagent_auth::{
    AuthRouterState, AuthState, auth_middleware, auth_routes, csrf_middleware, security_headers_middleware,
};
use veryagent_channel::channel_routes;
#[cfg(feature = "weixin")]
use veryagent_channel::weixin_login_route;
use veryagent_common::ApiErrorLogContext;
use veryagent_conversation::{conversation_ops_routes, conversation_routes};
use veryagent_cron::cron_routes;
use veryagent_extension::{extension_routes, hub_routes, skill_routes};
use veryagent_file::file_routes;
use veryagent_mcp::mcp_routes;
use veryagent_office::{office_proxy_routes, office_routes};
use veryagent_realtime::{WsHandlerState, ws_upgrade_handler};
use veryagent_shell::shell_routes;
use veryagent_system::{connection_test_routes, system_routes};
use veryagent_team::team_routes;
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    let router = router.layer(DefaultBodyLimit::max(veryagent_common::constants::BODY_LIMIT));
