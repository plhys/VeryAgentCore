...
/// the same binary. When `VERYAGENT_BUILTIN_SKILLS_PATH` is set, skip
...
pub(super) async fn materialize_builtin_skills(data_dir: &Path) -> Result<()> {
    let skip = std::env::var(veryagent_extension::BUILTIN_SKILLS_ENV_VAR)
        .map(|v| !v.is_empty())
        .unwrap_or(false);
    if skip {
        return Ok(());
    ...
    veryagent_extension::materialize_if_needed(
        data_dir,
        veryagent_extension::builtin_skills_corpus(),
