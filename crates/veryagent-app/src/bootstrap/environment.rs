...
use veryagent_app::AppConfig;
use veryagent_db::Database;
...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment, BootstrapError> {
    ...
    unsafe {
        std::env::set_var("VERYAGENT_WORK_DIR", &work_dir);
    }
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database, BootstrapError> {
    ...
    veryagent_db::maybe_copy_legacy_database(&db_path).map_err(|e| {
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            "data.legacy_db",
            "failed to initialize application data",
    ...
    let database = veryagent_db::init_database_staged(&db_path).await.map_err(|e| {
        let stage = e.stage();
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            stage,
...
mod tests {
    ...
    fn database_stage_comes_from_db_boundary_error() {
        let err = veryagent_db::DatabaseInitError::new(
            "database.migration",
            veryagent_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(42)),
        );
    ...
    fn database_schema_repair_stage_comes_from_db_boundary_error() {
        let err = veryagent_db::DatabaseInitError::new(
            ...
            veryagent_db::DbError::Init("repair failed".into()),
        );
