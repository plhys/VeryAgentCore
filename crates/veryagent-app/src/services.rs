...
use veryagent_auth::{CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret};
use veryagent_common::OnConversationDelete;
use veryagent_conversation::{ConversationService, runtime_state::ConversationRuntimeStateService};
use veryagent_db::{
...
use veryagent_realtime::{BroadcastEventBus, WebSocketManager};
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<veryagent_extension::SkillPaths>,
...
impl AppServices {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        let skill_paths = Arc::new(veryagent_extension::resolve_skill_paths(&app_resource_dir, &data_dir));
        veryagent_extension::sync_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
            .await
...
struct ConversationServiceDeps<'a> {
    ...
    skill_paths: Arc<veryagent_extension::SkillPaths>,
