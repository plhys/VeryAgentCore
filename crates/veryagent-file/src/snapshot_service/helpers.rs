...
pub(super) const SNAPSHOT_DIR_PREFIX: &str = "veryagent-snapshot-";
...
const SNAPSHOT_SIG_NAME: &str = "veryagent";
/// Signature email used for snapshot commits.
const SNAPSHOT_SIG_EMAIL: &str = "snapshot@veryagent.local";
...
pub(super) struct WorkspaceState {
    ...
    /// - snapshot mode: `/tmp/veryagent-snapshot-{hash}` (bare-style git dir).
