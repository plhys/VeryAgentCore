...
use veryagent_common::ApiError;
use veryagent_common::constants::UPLOAD_MAX_SIZE;
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `veryagent-app`
...
fn to_zip_entry(e: veryagent_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let mode = match info.mode {
        SnapshotMode::GitRepo => veryagent_api_types::SnapshotMode::GitRepo,
        SnapshotMode::Snapshot => veryagent_api_types::SnapshotMode::Snapshot,
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = veryagent_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, veryagent_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, veryagent_api_types::SnapshotMode::Snapshot);
        assert!(r.branch.is_none());
    }

    #[test]
    fn compare_response_conversion() {
        use veryagent_common::FileChangeOperation;
