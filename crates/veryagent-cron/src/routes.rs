...
async fn system_resume(
    ...
    let is_internal = headers.get("x-veryagent-internal").and_then(|value| value.to_str().ok()) == Some("1");
    if !is_internal {
...
mod tests {
    ...
    fn conversation_error_maps_through_boundary_mapper() {
        let err: ApiError =
            CronError::Conversation(veryagent_conversation::ConversationError::NotFound { id: "conv-1".into() }).into();
