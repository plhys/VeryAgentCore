...
use veryagent_ai_agent::task_manager::IWorkerTaskManager;
use veryagent_ai_agent::types::SendMessageData;
use veryagent_ai_agent::{AgentRegistry, AgentStreamEvent};
use veryagent_api_types::{AssistantConversationRequest, CreateConversationRequest, SendMessageRequest};
use veryagent_common::{
    AgentType, ProviderWithModel, WorkspacePathValidationError, now_ms, validate_workspace_path_availability,
};
use veryagent_conversation::{ConversationError, ConversationService};
use veryagent_db::models::MessageRow;
use veryagent_db::{ConversationRowUpdate, IConversationRepository};
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<veryagent_db::models::ConversationRow>, CronError> {
    ...
    pub async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<veryagent_db::models::ConversationAssistantSnapshotRow>, CronError> {
        self.conversation_repo
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: veryagent_common::now_ms(),
        };
...
impl JobExecutor {
    ...
    fn spawn_skill_suggest_flow(
        &self,
        agent: veryagent_ai_agent::AgentInstance,
...
mod tests {
    ...
    use veryagent_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
    use veryagent_ai_agent::protocol::events::FinishEventData;
    use veryagent_ai_agent::types::BuildTaskOptions;
    use veryagent_api_types::{AgentModeResponse, ConfigOptionConfirmation, SetConfigOptionResponse, WebSocketMessage};
    use veryagent_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use veryagent_db::{
    ...
    fn sample_job() -> CronJob {
        CronJob {
            ...
            agent_config: Some(CronAgentConfig {
                ...
                model: None,
                config_options: None,
                workspace: Some(ensure_named_workspace_path("veryagent-cron-sample-job-workspace")),
            }),
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(veryagent_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    async fn build_conversation_extra_preserves_agent_workspace() {
        ...
        assert_eq!(
            extra["workspace"],
            ensure_named_workspace_path("veryagent-cron-sample-job-workspace")
        );
    }
    ...
    async fn execute_inner_uses_conversation_workspace_when_job_workspace_missing() {
        ...
        assert_eq!(
            ...
            ensure_named_workspace_path("veryagent-cron-existing-conversation-workspace")
        );
    ...
    async fn execute_inner_persists_agent_workspace_when_conversation_workspace_missing() {
        ...
        assert_eq!(
            value["workspace"],
            ensure_named_workspace_path("veryagent-cron-agent-workspace")
        );
    }
    ...
    async fn execute_inner_inserts_right_side_user_message_for_cron_prompt() {
        ...
        let repo = Arc::new(MissingWorkspaceConversationRepo::new(
            "conv_1",
            serde_json::json!({
                "workspace": ensure_named_workspace_path("veryagent-cron-existing-conversation-workspace")
            }),
        ));
    ...
    fn make_executor_for_busy_tests() -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            async fn get_or_build_task(
                ...
                _: &str,
                _: BuildTaskOptions,
            ) -> Result<AgentInstance, veryagent_ai_agent::AgentError> {
                Err(veryagent_ai_agent::AgentError::internal("stub"))
            }
            fn kill(
                &self,
                _: &str,
                _: Option<veryagent_common::AgentKillReason>,
            ) -> Result<(), veryagent_ai_agent::AgentError> {
                Ok(())
            }
            fn kill_and_wait(
                &self,
            ...
            fn collect_idle(&self, _: veryagent_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            async fn get(&self, _id: &str) -> Result<Option<veryagent_db::models::ConversationRow>, veryagent_db::DbError> {
                Ok(None)
            }
            async fn create(&self, _row: &veryagent_db::models::ConversationRow) -> Result<(), veryagent_db::DbError> {
                Ok(())
            }
            async fn update(&self, _id: &str, _updates: &ConversationRowUpdate) -> Result<(), veryagent_db::DbError> {
                Ok(())
            }
            async fn delete(&self, _id: &str) -> Result<(), veryagent_db::DbError> {
                Ok(())
            }
            ...
            ) -> Result<PaginatedResult<veryagent_db::models::ConversationRow>, veryagent_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
            ...
            async fn find_by_source_and_chat(
                ...
                _chat_id: &str,
                _agent_type: &str,
            ) -> Result<Option<veryagent_db::models::ConversationRow>, veryagent_db::DbError> {
                Ok(None)
            }
            async fn list_by_cron_job(
                ...
                _user_id: &str,
                _cron_job_id: &str,
            ) -> Result<Vec<veryagent_db::models::ConversationRow>, veryagent_db::DbError> {
                Ok(vec![])
            }
            ...
            ) -> Result<MessagePageResult, veryagent_db::DbError> {
            ...
            async fn insert_message(&self, _message: &veryagent_db::models::MessageRow) -> Result<(), veryagent_db::DbError> {
                Ok(())
            }
            async fn update_message(&self, _id: &str, _updates: &MessageRowUpdate) -> Result<(), veryagent_db::DbError> {
                Ok(())
            }
            async fn delete_messages_by_conversation(&self, _conv_id: &str) -> Result<(), veryagent_db::DbError> {
                Ok(())
            }
            async fn get_message_by_msg_id(
                ...
                _msg_id: &str,
                _msg_type: &str,
            ) -> Result<Option<veryagent_db::models::MessageRow>, veryagent_db::DbError> {
                Ok(None)
            }
            async fn search_messages(
                ...
                _page: u32,
                _page_size: u32,
            ) -> Result<PaginatedResult<MessageSearchRow>, veryagent_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
        ...
        impl veryagent_realtime::EventBroadcaster for StubBroadcaster {
            fn broadcast(&self, _: WebSocketMessage<serde_json::Value>) {}
        }

        struct StubSkillResolver;
        #[async_trait::async_trait]
        impl veryagent_conversation::skill_resolver::SkillResolver for StubSkillResolver {
            ...
            async fn resolve_skills(
                &self,
                _names: &[String],
            ) -> Vec<veryagent_conversation::skill_resolver::ResolvedAgentSkill> {
                Vec::new()
            }
            ...
            async fn link_workspace_skills(
                ...
                _rel_dirs: &[&str],
                _skills: &[veryagent_conversation::skill_resolver::ResolvedAgentSkill],
            ) -> usize {
                0
            }
        }

        let stub_broadcaster: Arc<dyn veryagent_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn veryagent_db::IAgentMetadataRepository> = Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn veryagent_db::IAcpSessionRepository> = Arc::new(StubAcpSessionRepo);
    ...
    impl RecordingAgent {
        fn new(conversation_id: &str, mode: &str, initialized: bool) -> Self {
            ...
            Self {
                conversation_id: conversation_id.to_owned(),
                workspace: ensure_named_workspace_path("veryagent-cron-agent-workspace"),
                event_tx,
    ...
    impl IAgentTask for RecordingAgent {
        ...
        async fn send_message(&self, data: SendMessageData) -> Result<(), veryagent_ai_agent::AgentSendError> {
            self.send_calls.fetch_add(1, Ordering::Relaxed);
            self.sent_messages.write().await.push(data);
            Ok(())
        }

        async fn cancel(&self) -> Result<(), veryagent_ai_agent::AgentError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), veryagent_ai_agent::AgentError> {
            Ok(())
        }
    }
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, veryagent_ai_agent::AgentError> {
        ...
        async fn set_config_option(
            ...
            option_id: &str,
            value: &str,
        ) -> Result<SetConfigOptionResponse, veryagent_ai_agent::AgentError> {
            if option_id != "mode" {
                return Err(veryagent_ai_agent::AgentError::bad_request(format!(
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(&self, id: &str) -> Result<Option<veryagent_db::models::ConversationRow>, veryagent_db::DbError> {
            Ok(Some(veryagent_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "cron".into(),
    ...
    struct MissingWorkspaceConversationRepo {
        row: veryagent_db::models::ConversationRow,
        updates: Mutex<Vec<ConversationRowUpdate>>,
        inserted_messages: Mutex<Vec<veryagent_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<veryagent_db::models::MessageRow> {
    ...
    impl veryagent_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn update(&self, _id: &str, updates: &ConversationRowUpdate) -> Result<(), veryagent_db::DbError> {
        ...
        async fn insert_message(&self, message: &veryagent_db::models::MessageRow) -> Result<(), veryagent_db::DbError> {
        ...
        ) -> Result<ConversationArtifactRow, veryagent_db::DbError> {
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn veryagent_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    fn make_executor_with_task_manager_repo_and_broadcaster(
        task_manager: Arc<dyn IWorkerTaskManager>,
        repo: Arc<dyn IConversationRepository>,
        broadcaster: Arc<dyn veryagent_realtime::EventBroadcaster>,
    ) -> JobExecutor {
        struct StubSkillResolver;
    ...
    #[async_trait::async_trait]
    impl veryagent_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<veryagent_db::models::AcpSessionRow>, veryagent_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &veryagent_db::CreateAcpSessionParams<'_>,
        ) -> Result<veryagent_db::models::AcpSessionRow, veryagent_db::DbError> {
            Err(veryagent_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: &str,
            _session_id: &str,
        ) -> Result<bool, veryagent_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _conversation_id: &str) -> Result<bool, veryagent_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<veryagent_db::PersistedSessionState>, veryagent_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: &str,
            _params: &veryagent_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, veryagent_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl veryagent_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(&self) -> Result<Vec<veryagent_db::models::AgentMetadataRow>, veryagent_db::DbError> {
            Ok(Vec::new())
        }
        async fn get(&self, _id: &str) -> Result<Option<veryagent_db::models::AgentMetadataRow>, veryagent_db::DbError> {
            Ok(None)
        }
        async fn find_by_source_and_name(
            ...
            _agent_source: &str,
            _name: &str,
        ) -> Result<Option<veryagent_db::models::AgentMetadataRow>, veryagent_db::DbError> {
            Ok(None)
        }
        ...
        async fn upsert(
            &self,
            _params: &veryagent_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<veryagent_db::models::AgentMetadataRow, veryagent_db::DbError> {
            Err(veryagent_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &veryagent_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<veryagent_db::models::AgentMetadataRow>, veryagent_db::DbError> {
            Ok(None)
        }
        async fn update_availability_snapshot(
            &self,
            _id: &str,
            _params: &veryagent_db::models::UpdateAgentAvailabilitySnapshotParams<'_>,
        ) -> Result<Option<veryagent_db::models::AgentMetadataRow>, veryagent_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<(), veryagent_db::DbError> {
            Ok(())
        }
        async fn set_enabled(&self, _id: &str, _enabled: bool) -> Result<bool, veryagent_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _id: &str) -> Result<bool, veryagent_db::DbError> {
            Ok(false)
        }
    }
}
