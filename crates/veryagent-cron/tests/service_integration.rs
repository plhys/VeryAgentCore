...
use veryagent_ai_agent::AgentRegistry;
use veryagent_ai_agent::agent_task::AgentInstance;
...
use veryagent_common::{PaginatedResult, ProviderWithModel, TimestampMs, now_ms};
use veryagent_conversation::ConversationService;
use veryagent_conversation::response_middleware::{CronCreateParams, CronUpdateParams};
use veryagent_db::{
...
use veryagent_cron::events::CronEventEmitter;
use veryagent_cron::executor::JobExecutor;
use veryagent_cron::scheduler::CronScheduler;
use veryagent_cron::service::{CronService, CronServiceDeps};
use veryagent_cron::types::JobStatus;
...
impl veryagent_ai_agent::task_manager::IWorkerTaskManager for StubTaskManager {
    ...
    fn kill(&self, _: &str, _: Option<veryagent_common::AgentKillReason>) -> Result<(), veryagent_ai_agent::AgentError> {
        Ok(())
    }
    fn kill_and_wait(
        &self,
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<veryagent_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<String, veryagent_db::models::ConversationRow>>,
...
impl StubConvRepo {
    ...
    fn upsert_artifact_row(&self, artifact: veryagent_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<veryagent_db::ConversationArtifactRow> {
...
impl IConversationRepository for StubConvRepo {
    async fn get(&self, id: &str) -> Result<Option<veryagent_db::models::ConversationRow>, veryagent_db::DbError> {
        ...
        let row = if id == "conv_mode" {
            veryagent_db::models::ConversationRow {
                id: id.into(),
                user_id: "u1".into(),
                ...
                extra: serde_json::json!({
                    "backend": "gemini",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-gemini-workspace"),
                    "session_mode": "yolo",
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "hermes",
                    "agent_name": "Hermes",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-hermes-workspace"),
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "gemini",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-gemini-default-workspace"),
                    "session_mode": "default",
                    "current_model_id": "gemini-2.5-pro"
                })
                .to_string(),
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "codex",
                    "agent_name": "Codex",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-codex-workspace"),
                    "session_mode": "default",
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "claude",
                    "agent_name": "Claude",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-claude-workspace"),
                    "session_mode": "default",
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "claude",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-stale-backend-workspace"),
                    "session_mode": "yolo",
                    "current_model_id": "gemini-2.5-pro"
                })
                .to_string(),
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "anthropic",
                    "agent_name": "Aion CLI",
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-aionrs-workspace"),
                    "session_mode": "default",
                    "current_model_id": "claude-sonnet-4-20250514"
                })
                .to_string(),
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-assistant-stale-backend-workspace"),
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-missing-assistant-stale-backend-workspace")
                })
                .to_string(),
                pinned: false,
                pinned_at: None,
            ...
            veryagent_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("veryagent-cron-service-assistant-snapshot-workspace")
                })
                .to_string(),
                pinned: false,
                pinned_at: None,
    ...
    async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<ConversationAssistantSnapshotRow>, veryagent_db::DbError> {
        Ok(self.assistant_snapshots.lock().unwrap().get(conversation_id).cloned())
    }

    async fn create(&self, row: &veryagent_db::models::ConversationRow) -> Result<(), veryagent_db::DbError> {
        self.rows.lock().unwrap().insert(row.id.clone(), row.clone());
        Ok(())
    }
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), veryagent_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .entry(id.to_owned())
            .or_insert_with(|| veryagent_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "u1".into(),
                name: "stub".into(),
                r#type: "default".into(),
    ...
    async fn list_artifacts(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<veryagent_db::ConversationArtifactRow>, veryagent_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<veryagent_db::ConversationArtifactRow>, veryagent_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        artifact: &veryagent_db::ConversationArtifactRow,
    ) -> Result<veryagent_db::ConversationArtifactRow, veryagent_db::DbError> {
...
async fn setup_with_conv_repo() -> (
    ...
    let data_dir = std::env::temp_dir().join(format!("veryagent-cron-test-{}", now_ms()));
    ...
    let task_manager: Arc<dyn veryagent_ai_agent::task_manager::IWorkerTaskManager> = Arc::new(StubTaskManager);
...
fn make_create_req(name: &str, schedule: CronScheduleDto) -> CreateCronJobRequest {
    CreateCronJobRequest {
        ...
        execution_mode: None,
        agent_config: Some(veryagent_api_types::CronAgentConfigWriteDto {
...
async fn create_job_strips_legacy_agent_ids_when_assistant_id_present() {
    ...
    req.agent_config = Some(veryagent_api_types::CronAgentConfigWriteDto {
        name: "Helper".into(),
        cli_path: None,
...
async fn create_job_requires_assistant_id_for_new_jobs() {
    ...
    assert!(matches!(err, veryagent_cron::error::CronError::InvalidAgentConfig(_)));
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(err, veryagent_cron::error::CronError::JobNotFound(_)));
}
...
async fn cj9_update_schedule_type() {
    ...
    assert!(matches!(
        updated.schedule,
        veryagent_cron::types::CronSchedule::Cron { .. }
    ));
...
async fn cj12_delete_nonexistent() {
    ...
    assert!(matches!(
        err,
        veryagent_cron::error::CronError::Database(veryagent_db::DbError::NotFound(_))
    ));
}
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(veryagent_db::ConversationArtifactRow {
...
async fn sk4_save_empty_skill() {
    ...
    let err = svc
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, veryagent_cron::error::CronError::InvalidSkillContent(_)));
}
...
async fn sc5_invalid_cron_expression() {
    ...
    let err = svc.add_job(req).await.unwrap_err();
    assert!(matches!(err, veryagent_cron::error::CronError::InvalidCronExpression(_)));
}
...
async fn sc7_every_zero_interval() {
    ...
    let err = svc.add_job(req).await.unwrap_err();
    assert!(matches!(err, veryagent_cron::error::CronError::InvalidSchedule(_)));
}
...
async fn icron_service_create_job() {
    ...
    use veryagent_conversation::response_middleware::ICronService;
...
async fn icron_service_create_job_inherits_conversation_mode_without_backend() {
    ...
    assert_eq!(
        ...
        Some(ensure_named_workspace_path("veryagent-cron-service-gemini-workspace").as_str())
    );
}
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = veryagent_db::UpdateCronJobParams {
...
async fn cd3_on_conversation_delete_trait() {
    use veryagent_common::OnConversationDelete;
