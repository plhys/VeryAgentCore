...
use aionui_runtime::managed_resources::{
    ensure_dev_local_root, export_acp_tool_to_dev_local, export_node_runtime_to_dev_local, export_node_runtime_to_root,
};
use aionui_runtime::{ensure_managed_acp_tool, ensure_node_runtime, prepare_managed_acp_tool_to_root};
...
pub async fn run_prepare_managed_resources(args: PrepareManagedResourcesArgs) -> Result<ExitCode> {
    ...
    for tool in [ManagedAcpToolId::CodexAcp, ManagedAcpToolId::ClaudeAgentAcp] {
        let exported = match bundle_out.as_ref() {
            Some(_) => {
                prepare_managed_acp_tool_to_root(tool, &output_root)
                    .await
                    .with_context(|| format!("prepare managed {} artifact for bundle root", tool.display_name()))?
                    .root
            }
            None => {
                ...
                export_acp_tool_to_dev_local(&resolved.root, tool.slug(), tool.version(), platform)
