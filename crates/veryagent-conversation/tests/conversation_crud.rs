...
use veryagent_ai_agent::{AgentError, IWorkerTaskManager};
use veryagent_api_types::{
    CreateConversationRequest, ListConversationsQuery, UpdateConversationRequest, WebSocketMessage,
};
use veryagent_common::{AgentKillReason, AgentType, ConversationSource, ConversationStatus, TimestampMs};
use veryagent_conversation::skill_resolver::SkillResolver;
use veryagent_conversation::{ConversationError, ConversationService};
use veryagent_db::{SqliteConversationRepository, init_database_memory};
...
impl IWorkerTaskManager for NoopTaskManager {
    fn get_task(&self, _: &str) -> Option<veryagent_ai_agent::AgentInstance> {
        None
    }
    async fn get_or_build_task(
        &self,
        _: &str,
        _: veryagent_ai_agent::types::BuildTaskOptions,
...
async fn setup() -> (ConversationService, Arc<TestBroadcaster>, Arc<dyn IWorkerTaskManager>) {
    ...
    let agent_metadata_repo: Arc<dyn veryagent_db::IAgentMetadataRepository> =
        Arc::new(veryagent_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn veryagent_db::IAcpSessionRepository> =
        Arc::new(veryagent_db::SqliteAcpSessionRepository::new(db.pool().clone()));
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("veryagent-conversation-crud-test-project");
...
async fn t4_4_extra_merge_preserves_existing_keys() {
    let (svc, _, task_mgr) = setup().await;
    let dir = std::env::temp_dir().join("veryagent-conversation-crud-extra-merge");
...
async fn full_lifecycle_create_get_update_delete() {
    let (svc, broadcaster, task_mgr) = setup().await;
    let updated_workspace = std::env::temp_dir().join("veryagent-conversation-crud-updated-workspace");
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use veryagent_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(veryagent_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn veryagent_db::IAgentMetadataRepository> =
        Arc::new(veryagent_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn veryagent_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = veryagent_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
