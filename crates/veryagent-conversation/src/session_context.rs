...
use veryagent_ai_agent::shared_kernel::{ConfigKey, ConfigValue, ModeId, ModelId, PersistedSessionState};
use veryagent_ai_agent::types::BuildTaskOptions;
use veryagent_api_types::{AcpBuildExtra, AionrsBuildExtra, TeamSessionBinding};
use veryagent_common::{AgentType, WorkspacePathValidationError, validate_workspace_path_availability};
use veryagent_db::models::ConversationRow;
use veryagent_db::{IAcpSessionRepository, IAgentMetadataRepository};
...
impl<'a> SessionContextBuilder<'a> {
    ...
    async fn resolve_acp_identity(
        ...
        extra: &serde_json::Value,
        session_row: Option<&veryagent_db::models::AcpSessionRow>,
...
fn decode_persisted_session_state(state: veryagent_db::PersistedSessionState) -> PersistedSessionState {
...
mod tests {
    ...
    async fn legacy_agent_types_are_archived_before_runtime_context() {
        ...
        for (agent_type, extra) in [
            ...
            ("codex", serde_json::json!({ "workspace": "/tmp/veryagent-codex-history" })),
            (
                "openclaw-gateway",
