...
use veryagent_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
use veryagent_ai_agent::protocol::events::{AgentStreamEvent, ErrorEventData, FinishEventData, TextEventData};
use veryagent_ai_agent::types::{BuildTaskOptions, SendMessageData};
use veryagent_ai_agent::{
...
use veryagent_db::{MessagePageCursor, MessagePageDirection, MessagePageParams, MessagePageResult};
use veryagent_extension::{AssistantRuleDispatcher, ExtensionError};
...
impl IConversationRepository for MockRepo {
    async fn get(&self, id: &str) -> Result<Option<ConversationRow>, veryagent_db::DbError> {
        let rows = self.rows.lock().unwrap();
        Ok(rows.iter().find(|r| r.id == id).cloned())
    }

    async fn create(&self, row: &ConversationRow) -> Result<(), veryagent_db::DbError> {
    ...
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), veryagent_db::DbError> {
        ...
        let row = rows
            .iter_mut()
            .find(|r| r.id == id)
            .ok_or_else(|| veryagent_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, id: &str) -> Result<(), veryagent_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| r.id != id);
        if rows.len() == len_before {
            return Err(veryagent_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, veryagent_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, veryagent_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, veryagent_db::DbError> {
        Ok(vec![])
    }
    ...
    async fn delete_assistant_snapshot(&self, conversation_id: &str) -> Result<bool, veryagent_db::DbError> {
        let mut rows = self.assistant_snapshots.lock().unwrap();
        let before = rows.len();
        ...
        let items = match &params.direction {
            ...
            MessagePageDirection::Anchor { message_id } => {
                let anchor = matched
                    ...
                    .cloned()
                    .ok_or_else(|| veryagent_db::DbError::NotFound(format!("Message {message_id}")))?;
    ...
    async fn insert_message(&self, message: &MessageRow) -> Result<(), veryagent_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        messages.push(message.clone());
        Ok(())
    }

    async fn update_message(&self, id: &str, updates: &MessageRowUpdate) -> Result<(), veryagent_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        let message = messages
            .iter_mut()
            .find(|message| message.id == id)
            .ok_or_else(|| veryagent_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, conv_id: &str) -> Result<(), veryagent_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, veryagent_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_stale_runtime_messages(&self) -> Result<Vec<MessageRow>, veryagent_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .filter(|message| {
    ...
    async fn list_artifacts(&self, conversation_id: &str) -> Result<Vec<ConversationArtifactRow>, veryagent_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, veryagent_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn mark_skill_suggest_artifacts_saved(
        ...
        cron_job_id: &str,
        updated_at: TimestampMs,
    ) -> Result<Vec<ConversationArtifactRow>, veryagent_db::DbError> {
        let mut artifacts = self.artifacts.lock().unwrap();
    ...
    async fn delete_artifacts_by_conversation(&self, conversation_id: &str) -> Result<(), veryagent_db::DbError> {
        self.artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, veryagent_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("veryagent-conversation-service-test-project");
...
fn unique_test_workspace_path(label: &str) -> PathBuf {
    let workspace = std::env::temp_dir()
        .join(format!("veryagent-conversation-service-test-{label}"))
...
async fn insert_conversation_with_type(repo: &Arc<MockRepo>, user_id: &str, agent_type: AgentType) -> ConversationRow {
    let id = format!(
        ...
        veryagent_common::generate_short_id()
    );
...
async fn create_returns_conversation_with_defaults() {
    ...
    assert_eq!(events[0].data["source"], "veryagent");
}
...
async fn create_rejects_unavailable_workspace_with_trailing_whitespace_in_request() {
    let (svc, _broadcaster, _repo, _task_mgr) = make_service();
    let dir = std::env::temp_dir().join(format!("veryagent-test-{}", veryagent_common::generate_short_id()));
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, veryagent_api_types::ConversationRuntimeStateKind::Idle);
...
async fn update_extra_merge() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "veryagent-conversation-update-extra-merge-{}",
        veryagent_common::generate_short_id()
    ));
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            veryagent_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
async fn send_message_missing_workspace_persists_message_and_failure_tip() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    broadcaster.take_events();
    let legacy_workspace = format!("/tmp/does-not-exist-{}", veryagent_common::generate_short_id());
    repo.update(
        &conv.id,
        &ConversationRowUpdate {
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = veryagent_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn create_resolves_assistant_snapshot_and_updates_preferences() {
    ...
    assert_eq!(
        resp.assistant,
        Some(veryagent_api_types::ConversationAssistantIdentityResponse {
            id: "preset-1".into(),
            source: "builtin".into(),
            name: "Preset".into(),
            avatar: "🤖".into(),
