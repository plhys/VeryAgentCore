...
use veryagent_ai_agent::protocol::events::{ErrorEventData, TipType};
use veryagent_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use veryagent_api_types::{AgentErrorCode, WebSocketMessage};
use veryagent_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
...
use veryagent_db::IConversationRepository;
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(veryagent_ai_agent::protocol::events::FinishEventData::default()),
...
mod tests {
    ...
    use veryagent_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    ...
    impl SkillResolver for RecordingSkillResolverForRelay {
        ...
        async fn resolve_skills(&self, _names: &[String]) -> Vec<veryagent_extension::ResolvedAgentSkill> {
            Vec::new()
        }
        ...
        async fn link_workspace_skills(
            ...
            _rel_dirs: &[&str],
            _skills: &[veryagent_extension::ResolvedAgentSkill],
        ) -> usize {
            0
        }
    }
    ...
    async fn run_text_then_finish_persists_message() {
        ...
        let bus = Arc::new(veryagent_realtime::BroadcastEventBus::new(64));
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            veryagent_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: veryagent_ai_agent::protocol::events::TipType::Warning,
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(veryagent_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use veryagent_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use veryagent_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(veryagent_realtime::BroadcastEventBus::new(64));
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, veryagent_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, veryagent_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: &str) -> Result<Option<veryagent_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, _row: &veryagent_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(&self, _id: &str, _updates: &veryagent_db::ConversationRowUpdate) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &veryagent_db::ConversationFilters,
        ) -> Result<veryagent_common::PaginatedResult<veryagent_db::models::ConversationRow>, DbError> {
            Ok(veryagent_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<veryagent_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<veryagent_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            &self,
            _conv_id: &str,
            _params: &veryagent_db::MessagePageParams,
        ) -> Result<veryagent_db::MessagePageResult, DbError> {
            Ok(veryagent_db::MessagePageResult {
                items: vec![],
        ...
        async fn update_message(&self, id: &str, updates: &veryagent_db::MessageRowUpdate) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<veryagent_common::PaginatedResult<veryagent_db::MessageSearchRow>, DbError> {
            Ok(veryagent_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
