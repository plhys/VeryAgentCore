...
use veryagent_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
use veryagent_ai_agent::types::BuildTaskOptions;
use veryagent_ai_agent::{AgentAvailabilityFeedbackPort, AgentError, AgentInstance, AgentSendError, IWorkerTaskManager};
...
use veryagent_db::models::{ConversationRow, MessageRow};
use veryagent_db::{
...
use veryagent_extension::AssistantRuleDispatcher;
use veryagent_mcp::{AcpMcpCapabilities, parse_acp_mcp_capabilities};
use veryagent_realtime::EventBroadcaster;
use veryagent_runtime::{RuntimeCommandProbe, probe_node_runtime_supported, probe_runtime_command, resolve_command_path};
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a veryagent_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    /// `delete()` removes the conversation row. Used by `veryagent-app` to wire up `WorkerTaskManagerImpl`
...
impl ConversationService {
    ...
    /// to `veryagent`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let row = veryagent_db::models::ConversationRow {
            id: id.clone(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(veryagent_api_types::ConversationAssistantIdentityResponse {
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert(&veryagent_db::UpsertAssistantPreferenceParams {
                assistant_definition_id: &snapshot.assistant_definition_id,
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let user_msg = veryagent_db::models::MessageRow {
...
impl ConversationService {
    ...
    /// workspaces are constrained to VeryAgent's generated path; custom
...
fn classify_repo_mcp_status(row: &veryagent_db::models::McpServerRow, support: McpSupportPolicy) -> ConversationMcpStatus {
...
mod tests {
    ...
    fn enum_to_db_agent_type() {
        use veryagent_common::AgentType;
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::Aionui).unwrap(), "veryagent");
    ...
    fn response_with_type(agent_type: veryagent_common::AgentType) -> ConversationResponse {
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_veryagent_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
