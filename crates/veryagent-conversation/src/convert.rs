...
use veryagent_db::MessageSearchRow;
use veryagent_db::models::ConversationAssistantSnapshotRow;
use veryagent_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
///
/// VeryAgent stores the full provider object (`TProviderWithModel`) which includes
...
mod tests {
    use super::*;
    use veryagent_common::{AgentType, ConversationSource, ConversationStatus};
    use serde_json::json;
    ...
    #[test]
    fn parse_provider_with_model_veryagent_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("veryagent"),
            None,
            r#"{"workspace":"/srv/veryagent-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/veryagent-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("veryagent"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/veryagent-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("veryagent".into()),
