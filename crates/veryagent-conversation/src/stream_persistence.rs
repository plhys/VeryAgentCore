...
use veryagent_ai_agent::protocol::events::{
    ErrorEventData, TipType, TipsEventData,
    tool_call::{AcpToolCallStatus, ToolCallStatus},
};
use veryagent_api_types::{ConversationRuntimeSummary, WebSocketMessage};
use veryagent_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
use veryagent_db::models::MessageRow;
use veryagent_db::{ConversationRowUpdate, DbError, IConversationRepository, MessageRowUpdate};
...
impl StreamPersistenceAdapter {
    ...
    #[tracing::instrument(skip_all)]
    pub async fn persist_tool_call(&self, data: &veryagent_ai_agent::protocol::events::tool_call::ToolCallEventData) {
    ...
    pub async fn persist_acp_tool_call(
        &self,
        data: &veryagent_ai_agent::protocol::events::tool_call::AcpToolCallEventData,
    ) {
    ...
    #[tracing::instrument(skip_all)]
    pub async fn persist_tool_group(&self, entries: &[veryagent_ai_agent::protocol::events::tool_call::ToolGroupEntry]) {
