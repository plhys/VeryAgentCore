use veryagent_db::{init_database, init_database_memory, maybe_copy_legacy_database};
use sqlx::Row;
...
async fn system_user_has_valid_timestamps() {
    let before = veryagent_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = veryagent_common::now_ms();
...
fn copy_legacy_noop_when_no_legacy_db() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("veryagent-backend.db");
...
fn copy_legacy_noop_when_target_exists() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("veryagent-backend.db");
    let legacy = dir.path().join("veryagent.db");
...
fn concurrent_init_database_does_not_panic_on_unique_conflict() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("veryagent-backend.db");
    ...
    let lock = path.with_file_name("veryagent-backend.db.migrate.lock");
