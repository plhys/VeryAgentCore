...
use veryagent_system::ClientPrefService;
...
pub(crate) async fn load_stt_config(
...
) -> Result<SpeechToTextConfig, veryagent_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: veryagent_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(veryagent_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = veryagent_system::ClientPrefService::new(repo);
