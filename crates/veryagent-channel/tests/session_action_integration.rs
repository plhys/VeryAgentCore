...
use veryagent_common::{generate_id, now_ms};
use veryagent_db::models::AssistantUserRow;
use veryagent_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
use veryagent_realtime::EventBroadcaster;

use veryagent_channel::action::{ActionExecutor, MessageResult};
use veryagent_channel::channel_settings::ChannelSettingsService;
use veryagent_channel::pairing::PairingService;
use veryagent_channel::session::SessionManager;
use veryagent_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn veryagent_db::IClientPreferenceRepository> =
        Arc::new(veryagent_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
// veryagent-db sqlite_channel.rs::update_session_conversation_persists.
